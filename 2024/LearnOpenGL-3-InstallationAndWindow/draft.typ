...
在这四个参数中，前两个参数控制窗口左下角的位置. 第三个和第四个参数控制渲染窗口的宽度和高度（像素）.

#in-janim[
  大概弄一个带有滑动条的演示，滑动条控制上面四个参数
]

我们将视口的大小设置得比GLFW窗口的小，这样子之后所有的OpenGL渲染将会在一个更小的窗口中显示，这样子的话我们也可以将一些其它元素显示在OpenGL视口之外.
...
这里我们创建一个 process_input 函数，我们把处理输入的代码都写在这里面，保持这个文件里代码的整洁
...
我们要把所有的渲染操作放到这个循环中，因为我们想让这些渲染指令在每次渲染循环迭代的时候都能被执行

我们把渲染操作的代码插到这里

为了测试一些都正常工作，我们使用一个自定义的颜色清空屏幕. 在每个新的渲染迭代开始的时候我们总是希望清屏，否则我们仍能看见上一次的渲染结果（这可能是你想要的效果，但通常来说不是）. 我们可以使用 `ctx.clear` 来清空屏幕的颜色缓冲，简单来说，我们可以给这个函数传入0到1之间的RGB颜色分量，比如 `(0.0, 1.0, 0.0)` 就是完全只有绿色分量，表现为纯绿色，在这里我们写上 `(0.2, 0.3, 0.3)` 这个颜色，可以看到我们将屏幕设置为了类似黑板的深蓝绿色
...
在这一节中，我们创建了基本的窗口，了解了如何处理用户按键输入，并调整了窗口的填充颜色
