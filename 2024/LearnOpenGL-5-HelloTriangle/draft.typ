...
与通常的屏幕坐标不同，y 轴正方向为向上，$(0,0)$ 坐标是屏幕的中心，而不是左上角

这意味着，屏幕的最左边就是 $x=-1.0$，最右边就是 $x=1.0$；同理，最下边是 $y=-1.0$，最上#pin(1)边是 $y=1.0$
...
为了将这组顶点数据发送给 GPU，我们通过#highlight[顶点缓冲对象(Vertex Buffer Objects, VBO)]管理这个内存，它会在 GPU 内存（通常被称为显存）中储存大量顶点。使用这些缓冲对象的好处是我们可以一次性的发送一大批数据到显卡上，而不是每个顶点发送一次
