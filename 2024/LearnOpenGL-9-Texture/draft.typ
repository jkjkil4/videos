...
为了能够把纹理映射(Map)到三角形上，我们需要指定三角形的每个顶点各自对应纹理的哪个部分。这样每个顶点就会关联着一个纹理坐标(Texture Coordinate)，用来标明该从纹理图像#pin(0)的哪里采样颜色。
...
纹理坐标在x和y轴上，范围为0到1之间（注意我们使用的是2D纹理图像）。使用纹理坐标获取纹理颜色叫做采样(Sampling)。#pin(1)
...
现在，我们把三角形左下角顶点的纹理坐标设置为 $(0,0)$，这样它就能对应纹理的左下角；同理将右下方的顶点设置为 $(1,0)$；上顶点的坐标设置为 $(0.5, 1.0)$ 对应纹理的上中位置。我们只要给顶点着色器传递这三个纹理坐标就行了，在片段插值的帮助下，我们的三个顶点会经过插值后传入片段着色器，产生三角形区域内每个点的纹理坐标信息。
...
这个行为在 ModernGL 中可以通过纹理的 `.repeat_x` 和 `.repeat_y` 进行控制，这意味着你可以对纹理的两个方向分别设置重复方式。

他们默认都是 `True`，当你把其中一个方向的行为设置为 `False` 后，这个方向上超出的部分会重复边缘的颜色，产生一种边缘被拉伸的效果，这也就是对应 `GL_CLAMP_TO_EDGE` 的行为。
...
想象一下（？），假设我们有一个包含着上千个物体的大房间，每个物体上都有纹理。
...
在渲染中切换多级渐远纹理级别(Level)时，OpenGL 在两个不同级别的多级渐远纹理层之间会产生不真实的硬边界。就像普通的纹理过滤一样，切换多级渐远纹理级别时你也可以在两个不同多级渐远纹理级别之间使用 `NEAREST` 和 `LINEAR` 过滤。为了指定不同多级渐远纹理级别之间的过滤方式，你可以使用下面四个选项中的一个代替原有的过滤方式：
...
一个常见的错误是，将放大过滤的选项设置为多级渐远纹理的选项之一。这样没有任何效果，因为多级渐远纹理主要是使用在纹理被缩小的情况下的，纹理放大不会使用多级渐远纹理。#pin(0)

#lp(0)[
  在多级渐远纹理往上放几个红色虚线矩形框表示没有放大的多级渐远纹理
]

== 加载与创建纹理

在前面我们学习了关于纹理的基本知识，了解了纹理环绕方式、纹理过滤以及多级渐远纹理。
