...
在“你好，三角形”这节的教程中提到，着色器(Shader)是运行在 GPU 上的小程序。这些小程序为图形渲染管线的某个特定部分而运行。

从基本意义上来说，着色器只是一种把输入转化为输出的程序。着色器也是一种非常独立的程序，因为它们之间不能相互通信；它们之间唯一的沟通只有通过输入和输出。

前面的教程里我们简要地触及了一点着色器的皮毛，并了解了如何恰当地使用它们。现在我们会用一种更加广泛的形式详细解释着色器，特别是 OpenGL 着色器语言(GLSL)。

== GLSL

着色器是使用一种类似C语言，叫作 GLSL 的语言写成的。GLSL 是为图形计算量身定制的，它包含一些针对向量和矩阵操作的有用特性。#pin(0)
...
通常情况下它至少会返回16个，大部分情况下够用了。

== 数据类型

和其它编程语言一样，GLSL 有数据类型来指定变量的种类。

GLSL 中包含 C 等其它语言大部分的基础数据类型：`int`、`float`、`double`、`uint` 和 `bool`。#pin(0)

#p(0)[
  用箭头指出它们的中文含义，不用读出来
]

GLSL 也有两种容器类型，它们会在这个教程中使用很多，分别是向量(Vector)和矩阵(Matrix)，其中矩阵我们会在之后的教程里再讨论。

=== 向量

GLSL 中的向量是一个可以包含由 2、3 或者 4 个分量的容器，分量的类型可以是前面默认基础类型的任意一个。

比如我们在之前绘制三角形的时候提到的，`vecn` 就是包含 `n` 个 float（浮点数）分量的向量，例如 `vec2` 就是包含 2 个浮点数分量，`vec3` 就是包含 3 个。

同样的，还有 `bvecn`、`ivecn`、`uvecn` 和 `dvecn` 分别对应不同数据类型的向量，根据需要使用即可，但是这些类型向量中，我们最常用到的还是包含浮点数类型的 `vecn`。
...
#note[
  在 C++ 里调用 OpenGL 可能更倾向于使用类似于 `layout (location = 0)`  的标识符，但是这里我们使用 ModernGL 推荐的方式
]
...
为了展示这具体是如何工作的，我们会稍微改动一下之前教程里的那个着色器，让顶点着色器为片段着色器决定颜色。
...
由于我们在顶点着色器中将颜色设置为深红色，最终的片段也是深红色的，所以我们运行程序就会有这样的结果：（录屏）

这一节的代码放在这个链接里了，有什么问题的话可以参考（贴链接）

这样我们成功地从顶点着色器向片段着色器发送了数据。让我们更上一层楼，看看能否从应用程序中直接给片段着色器发送一个颜色！

== Uniform

Uniform 是另一种从我们的应用程序在 CPU 上传递数据到 GPU 上的着色器的方式，但 uniform 和顶点数据有些不同。首先，uniform 是全局的，它可以被着色器程序的任意着色器阶段访问。其次，无论你把 uniform 值设置成什么，uniform 会一直保存他们的数据，知道它们被重置或更新。

要在 GLSL 中声明 uniform，我们只需将 `uniform` 关键字添加到具有类型和名称的着色器中。这样我们就可以在着色器中使用新声明的 uniform。
...
这里我们在绘制三角形前，先更新 uniform 的值，如果你正确更新了 uniform，你会看到你的三角形逐渐由绿变黑再变回绿色：（录屏）
