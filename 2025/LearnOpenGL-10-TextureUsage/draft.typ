...
此时，只有基本级别(Base-level)的纹理图像被加载了，如果要使用多级渐远纹理，我们需要使#pin(2)用 `texture.build_mipmaps()` 来生成它们，这会自动生成所有需要的多级渐远纹理。
...
后面的这部分我们会使用 `第6节 索引缓冲对象` 中使用索引缓冲对象(IBO)绘制矩形的代码，你可以把那个代码复制进来（贴个链接），我们以这个为基础来给矩形附着上纹理渲染。
...
我们便可以使用 GLSL 内置的 `texture` 函数来采样纹理的颜色，它第一个参数是纹理采样器，第二个参数是对应的纹理坐标。`texture` 函数会使用之前设置的纹理参数对相应的颜色值进行采样。这样就使得片段着色器的输出是，在对应纹理坐标上，经过纹理过滤后的颜色。
...
我们现在需要载入并创建另一个纹理，这里我们使用一张“你学习 OpenGL 时的面部表情”图片。注意这个图片是 `.png` 格式的，它还带有一个alpha不透明度通道，读取进来的 `img2` 是 `RGBA` 格式，带有四个通道，所以我们使用 `components=4` 给 OpenGL 注明一下
