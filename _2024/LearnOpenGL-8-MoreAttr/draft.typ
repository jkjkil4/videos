...
在前面的教程中，我们了解了如何填充 VBO、以及如何将着色器程序与 VBO 绑定为一个 VAO。

之前的这个顶点数据指定了三个顶点的位置，这次，我们同样打算把颜色数据加进顶点数据中。

我们把颜色数据以 3 个 float 值的形式写到 `vertices` 数组中，这样把三角形的三个角分别指定为红色、绿色和蓝色：
...
可以发现，这里我们加了一个对 `in_color` 属性的绑定，这表明将顶点数据传递给顶点着色器时，首先读取一个位置属性，然后还要再读取一个颜色属性，作为一个顶点的输入，依照这样的方式读取完三个顶点的属性。
