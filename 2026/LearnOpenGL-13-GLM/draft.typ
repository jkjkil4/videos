...
我们来做些更有意思的事情，让我们来缩放和旋转之前那个教程中的“开心的箱子”
...
需要留意的是，因为 `glm` 的这些函数是依次右乘的，而计算是依次向左，所以当我们把最终的 `trans` 矩阵作用到向量上时，效果是先缩放（`scale`） 再旋转（`rotate`）#note[分别高亮矩阵示意以及代码]，和代码的顺序不同。

我们现在有了最终的矩阵 `trans`，下一个大问题是：如何把矩阵传递给着色器？我们之前接触过 GLSL 中的向量类型，`vec2` `vec3` `vec4`，同样，GLSL 中也有矩阵类型，即 `mat2` `mat3` `mat4`，这里我们使用 `mat4` 类型因为我们用的是 4x4 矩阵。

我们修改顶点着色器让其接收一个 `mat4` 类型的 `uniform` 变量，再用矩阵 uniform 乘以位置向量，然后在代码中就可以设置这个 `uniform` 的矩阵值了。这里为了将 GLM 的矩阵传递作为 ModernGL 的 uniform 值，我们直接使用 `.write` 写入矩阵的字节数据即可。
...
接下来运行代码，如果你做对了，你将看到这样的结果：

#align(center)[（实机演示）]

我们做到了实现一个位移过的箱子，并且它会一直转，一个变换矩阵就做到了！现在你能感觉到为什么矩阵在图形领域是一个如此重要的工具了。我们可以定义无限数量的变换，而把他们组合为仅仅一个矩阵，并且如果愿意的话我们还可以重复使用它。在着色器中使用矩阵可以省去重新定义顶点数据的功夫，它也能够节省处理时间，因为我们没有一直重新发送我们的顶点数据（这是个非常慢的过程）。
